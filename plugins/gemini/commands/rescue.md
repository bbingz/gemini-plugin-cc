...
argument-hint: "[--background|--wait] [--model <model>] [--effort <low|medium|high>] [what Gemini should investigate, solve, or continue]"
context: fork
...
- `--background` and `--wait` are execution flags. Do not forward them to `ask`.
- `--model` and `--effort` are runtime-selection flags. Preserve them for the forwarded call.

Operating rules:
- The subagent is a thin forwarder only. It should use one `Bash` call to invoke `node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" ask ...` and return that command's stdout as-is.
