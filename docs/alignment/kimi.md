...
Kimi 侧已 merge `kimi-plugin-cc@8e18587`（10 files, +617/−46），完整计划文档在对侧 `docs/superpowers/plans/2026-04-21-alignment-response-gemini-v0.6.0.md`。本节只记录我方收到的结论，不跨写对侧文档。

### 8.1 差距处置

| 发现 | 对侧处置 | 备注 |
|---|---|---|
| P0 — Timing stub 误导性读取路径 | **已应用** | 删掉 `appendTimingHistory` 死路径，v0.1 改为"缺席即诚实信号"，不再用 stub 假装 |
| P1 — 主力模型使用验证（A 卷） | **延后 v0.2** | 阻塞在 kimi-cli 1.37 的 per-model usage re-probe；rationale 在对侧 `lessons.md §I.1` |
| P2 — `tests/` 目录缺席 | **延后 v0.2** | 与 timing 实现路径绑定一起建，避免重复返工 |
| P3 — 命令 contract 规范化 | **已应用** | rescue.md `argument-hint` 改为 provider-neutral / 8 个命令统一单引号 / `[--background|--wait]` 语法统一 |
| §5.2 CLI `/auth` 边界 | **已澄清** | `kimi-cli-runtime/SKILL.md` 明确：插件对 Moonshot auth 零耦合，`~/.kimi/credentials/` 100% CLI 自管 |
| §5.3 Phase-5 timing gate | **已澄清** | `lessons.md §I.1` 记录门控条件：1.37 re-probe → 若暴露 per-model 则走 6 段 gemini-scaffold，否则走 CLI-agnostic 3 段子集 |
| §5.5 `prompts.mjs` 模块 rationale | **已应用** | 加了 module-level 注释，指向 `review.mjs` 作为 review-flow prompt 的使用入口 |
| §2 `gfg-` foreground-job prefix | **有意跳过** | 对侧 rationale：unified-timing-path 的必要性依赖于"在做 timing"，在 v0.1 不采集 timing 的前提下没有理由加前缀；不视为差距 |

### 8.2 我方报告的事实纠正

- **§2 对照表第 13 行 "首行噪声截取 ❓ 未确认"** → **N/A for kimi**。Kimi CLI 从 byte 0 开始就是 clean JSONL（对侧 `probe-results.json v3` 证实 `top_level_keys_observed: [role, content]`）；首行噪声前缀是 **gemini CLI 独有的 quirk**，不是 sibling-generic 问题。
- **§1 "Phase 1 in progress" 推断** → 是 stale sub-CHANGELOG drift。v0.1 实际在我写报告时已 merge（对侧 commit `0bb38bf`），我读到的 `plugins/kimi/CHANGELOG.md` 是未同步的子 changelog，不是事实。

### 8.3 对侧给我方下一轮 baseline.md 的三条输入

1. **§6.3 主力模型使用验证** 需要加 "CLI must emit per-model usage in result event" 的 **prerequisite caveat** ——不是所有 sibling CLI 都吐（kimi 1.x / mini-agent 当前都不吐）
2. **"Stale sub-CHANGELOG drift"** 是 sibling-generic 陷阱（我的 "Phase 1 in progress" 误读就是它造成的），值得在 baseline 加一行说明：插件内部 CHANGELOG vs. 仓库根 CHANGELOG 的权威性排序
3. **Review 架构拆分形状**：如果我将来把 review 从 `gemini.mjs` 抽出，kimi 的"薄 CLI-specific adapter（`callKimiReview` / `callKimiAdversarialReview`）→ 厚共享 pipeline（`runReviewPipeline` in `scripts/lib/review.mjs`）"是值得参考的形状

### 8.4 我方后续动作

- §8.2 的事实纠正**只保留在 §8**，不回改 §2 行（保留当时快照，便于审计）
- **P1 / P2 不再点名**——分别属于上游 probe 阻塞 / v0.2 绑定，重复出现没信息量
- **baseline.md 的三条输入**另开一轮单独处理（不属于 loop-close 范畴）
